...
gcc ${CFLAGS} -O0 teste.c -o teste_SemOtimiz ${LFLAGS}
for k in $METRICA
do
    likwid-perfctr -C ${CPU} -g ${k} -m ./teste_SemOtimiz >${k}_SemOtimiz.log
done

rm -f teste
gcc ${CFLAGS} -O3 teste.c -o teste_Otimiz ${LFLAGS}
for k in $METRICA
do
    likwid-perfctr -C ${CPU} -g ${k} -m ./teste_Otimiz >${k}_Otimiz.log
done
